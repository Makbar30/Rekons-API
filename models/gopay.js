...
exports.insertImportGopay = values => {
    return new Promise(resolve => {
        ...
        WHERE NOT EXISTS (
        SELECT transaction_id FROM transaction_import WHERE transaction_id = '${values[5]}' AND channel = 'GOPAY'
        ) LIMIT 1`;
...
exports.insertdataMPGOPAY = data => {
    return new Promise(resolve => {
        ...
        SELECT * 
        FROM (
            SELECT  ${parseInt(data.bill_reff)} as bill_no, '${data.username_pengirim_gopay}' as sender , '${data.username_penerima_gopay}' as receiver , 'gopay' as channel , '${data.trx_id}' as transaction_id , CAST('${data.bill_date}' AS datetime) as tgl_transaksi ,
        ...
        FROM transaction_mp 
        WHERE transaction_id = '${data.trx_id}' AND channel = 'gopay'
        ) 
        LIMIT 1`;
        mysqlCon.query(sql, function (error, rows, fields) {
            if (error) {
