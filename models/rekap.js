...
exports.getRekapBank = () => {
    return new Promise(resolve => {
        ...
        FROM(
            SELECT 
            tr.bank_penerima as nama_bank, count(tr.bank_penerima) as jumlah_transaksi, 
...
exports.getRekapMasjidByReceiver = receiver => {
    return new Promise(resolve => {
        const sql = `SELECT 
        a.* , 
        (a.nominal_transaksi_awal - (a.nominal_potongan_kmdn + a.nominal_potongan_cashback + a.nominal_potongan_channel)) as total_akhir
...
exports.getRekapTrImport = () => {
    return new Promise(resolve => {
        const sql = `SELECT 
        a.*, 
        (a.payment_amount - (a.nominal_potongan_channel + a.nominal_potongan_kmdn + a.nominal_potongan_cashback)) as nominal_akhir  
        FROM (
            SELECT 
                tr.*, 
                (tr.payment_amount * tr.potongan_channel) as nominal_potongan_channel , 
                (tr.payment_amount * tr.potongan_kmdn) as nominal_potongan_kmdn, 
                (tr.payment_amount * tr.potongan_cashback) as nominal_potongan_cashback  
...
exports.getNominalDataImport = channel => {
    return new Promise(resolve => {
        var sql = `SELECT 
        IFNULL(COUNT(id),0) as jumlah_transaksi, 
        IFNULL(SUM(tr.payment_amount),0) as amount 
        from transaction_import tr
        WHERE tr.channel = '${channel}' `;
        console.log(sql)
        mysqlCon.query(sql,
...
exports.getNominalDataMP = channel => {
    return new Promise(resolve => {
        var sql = `SELECT 
        IFNULL(COUNT(id),0) as jumlah_transaksi, 
        IFNULL(SUM(tr.total_pembayaran),0) as amount 
        from transaction_mp tr
        WHERE tr.channel = '${channel}' `;
        console.log(sql)
        mysqlCon.query(sql,
...
exports.getDatabyrek = (receiver, start_date, end_date) => {
    return new Promise(resolve => {
        const sql = `SELECT 
        a.*, 
        (a.total_pembayaran - (a.nominal_potongan_kmdn + a.nominal_potongan_cashback + a.nominal_potongan_channel)) as total_akhir
...
exports.getDatabyMasjid = receiver => {
    return new Promise(resolve => {
        const sql = `SELECT 
        a.*, 
        (a.total_pembayaran - (a.nominal_potongan_kmdn + a.nominal_potongan_cashback + a.nominal_potongan_channel)) as total_akhir
...
exports.getDatabyMasjidAndDate = (nama_penerima, start_date, end_date) => {
    return new Promise(resolve => {
        ...
        FROM(
                ...
                FROM transaction_mp tr
                LEFT JOIN transaction_import ti ON ti.reference_id = tr.reference_id
                WHERE (tr.receiver = "${nama_penerima}" AND (tr.isTransfer = "F" AND (tr.tgl_transaksi BETWEEN "${start_date}" AND "${end_date}")))
            )a`;
        console.log(sql)
        mysqlCon.query(sql,
            function (error, rows, fields) {
